...
# Function to display and animate the jar filling using a bar
def animate_jar(correct_answers, total_questions):
    # Calculate fill percentage
    fill_percentage = (correct_answers / total_questions) * 100

    # Create a plot for the animated jar (using a bar for simplicity)
    fig, ax = plt.subplots()
    ax.barh([0], [fill_percentage], color='blue')  # Horizontal bar
    ax.set_xlim(0, 100)
    ax.set_ylim(-1, 1)
    ax.set_title(f"Jar Fill: {fill_percentage:.0f}% Full")
