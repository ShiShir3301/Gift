...
# Function to display and animate the "box" filling
def animate_box(correct_answers, total_questions):
    # Define box dimensions (cylinder-like shape)
    box_width = 2  # Width of the box (cylinder)
    box_height = 4  # Height of the box (cylinder)
    fill_height = (correct_answers / total_questions) * box_height  # Calculate fill height

    # Create a figure and axis
    fig, ax = plt.subplots(figsize=(6, 6))

    # Draw the "cylinder" (rounded rectangle to simulate cylindrical shape)
    ax.add_patch(patches.Rectangle((0, 0), box_width, box_height, linewidth=2, edgecolor='black', facecolor='lightgray', lw=2, zorder=1, joinstyle="round"))

    # Draw the filled part of the cylinder in red
    ax.add_patch(patches.Rectangle((0, 0), box_width, fill_height, linewidth=0, edgecolor='black', facecolor='red', zorder=2))

    # Set the limits of the plot to match the box size
...
def main():
    ...
    # Display the box animation based on the correct answers so far
    if answer_revealed:
        animate_box(st.session_state.correct_answers, len(questions))
