...
def slowly_fill_cylinder(correct_answers, total_questions):
    # Set the target fill height (100% of cylinder height)
    target_fill_height = 4
    current_fill_height = (correct_answers / total_questions) * 4
    increment = 0.05  # Small increments to create a smooth slow fill effect

    # Create a container to hold the figure update
    cylinder_plot = st.empty()

    # Gradually fill the cylinder over time
    while current_fill_height < target_fill_height:
        fig, ax = plt.subplots(figsize=(6, 8))

        # Draw the cylinder (as a rectangle)
        ax.add_patch(patches.Rectangle((0, 0), 2, 4, linewidth=2, edgecolor='black', facecolor='lightgray'))

        # Draw the filled part of the cylinder (in red)
        ax.add_patch(patches.Rectangle((0, 0), 2, current_fill_height, linewidth=0, edgecolor='black', facecolor='red'))

        # Update the limits and remove axes for clarity
        ax.set_xlim(0, 2)
        ax.set_ylim(0, 4)
        ax.axis('off')

        # Display the plot within the container
        cylinder_plot.pyplot(fig)

        # Increase the fill height gradually
        current_fill_height += increment

        # Wait for a short moment before the next increment to create a smooth animation effect
...
def main():
    ...
    # Once the user finishes, fill the cylinder slowly to 100% if it's not 10/10
    if st.session_state.question_index == len(questions) and st.session_state.correct_answers != len(questions):
        slowly_fill_cylinder(st.session_state.correct_answers, len(questions))
